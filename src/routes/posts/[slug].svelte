...
{#if post.coverImage?.url}
	<div class="sm:-mx-5 md:-mx-10 lg:-mx-20 xl:-mx-38 mb-5">
		<img class="rounded-lg" src={post.coverImage?.url} alt={`Cover Image for ${post.title}`} />
	</div>
{/if}
...
<a href={'/'} class="inline-flex items-center mb-3">
	<img
		src={post.author?.picture.url ? post.author?.picture.url : `/default-profile-pic.png`}
